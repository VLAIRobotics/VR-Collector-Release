...
`xarm collect` 启动后，数据录制由 **recorder_node** 负责。它订阅机械臂状态、VR 动作、相机图像，按帧率同步采集并存储为 **LeRobot v3.0 标准格式**数据集。本节介绍录制的实际操作与注意事项。

### 7.1 录制前提：相机话题需提前发布

SDK **不含相机驱动**。若配置中启用了相机（默认启用胸部相机），必须在**另一个终端**先启动相机节点，把对应话题发布出来：

- 胸部相机（默认必需）：`/cam_chest/cam_chest/color/image_raw`

否则录制时每帧都会提示 `Missing data: ['observation.images.cam_chest']`，一帧都存不进去。

> 若只想录关节与动作、不要图像，可在配置中将 `cameras.chest.enabled` 设为 `false`，进入**纯状态录制**，此时无需启动任何相机。

### 7.2 键盘控制

录制在 recorder 所在终端用键盘控制：
...
```text
r  →  用 VR 操作机械臂完成一次任务  →  n   （保存第 1 条 episode）
r  →  完成第二次任务               →  n   （保存第 2 条 episode）
... 重复 ...
q  →  保存并退出
```
...
数据采集分两步：按 `n` 只写入了原始数据，**原数据与视频要靠退出时的收尾（finalize）才保存**。退出不干净会留下**无法加载的半成品数据集**。

|           做法            |              结果              |
|---------------------------|--------------------------------|
| ✅ 按 `q` 退出             | 最稳，主动收尾后干净退出        |
| ✅ 按**一次** `Ctrl+C` 后耐心等 | 安全（关闭时有 60 秒收尾宽限） |
| ❌ 连续猛按 `Ctrl+C` / 直接关终端 | 打断收尾，产生半成品数据集 |

**录完数据优先按 `q`；如果用 `Ctrl+C` 只按一次、别连按，否则会损坏数据集**

### 7.4 采集的数据内容

|              字段                |          内容            |                  维度                 	                 |
|----------------------------------|-------------------------|------------------------------------------|
| `observation.state`              | 机械臂实际关节状态       | 单臂 8 维 / 双臂 16 维（7 关节 + 1 夹爪） |
| `action`                         | VR 遥操作的动作指令      | 同上                          			                  |
| `observation.images.cam_chest`   | 胸部相机图像（video）    | (3, 480, 640)                   	|
| `observation.images.cam_wrist_*` | 手腕相机图像（可选）     | (3, 480, 848)                   		|

### 7.5 数据存储与多次录制

**存储路径** = `dataset_path` + `repo_id`
例：`~/lerobot_datasets` + `xarm/pick_and_place` → 数据存到 `~/lerobot_datasets/xarm/pick_and_place/`。

|               目的                   |                	做法                                           	       |
|--------------------------------------|---------------------------------------------------|
| 同一任务多次采集、**累积到一个数据集** | 不改配置，重复 `xarm collect`，episode 自动接续编号 |
| 开一个**新的独立数据集**（换任务）     | 修改 `repo_id`，自动分到新文件夹     		       |
| 更换存储位置（如换硬盘）               | 修改 `dataset_path`                		   |

> 多次启动 `xarm collect` 时，若同一 `repo_id` 的数据集已存在且完整，会自动加载并续录；若上次退出不干净留下半成品，则会清晰报错以保护数据，此时请备份后删除该数据集目录或更换 `repo_id`。
...
### 8.4 VR 头显连不上 PC（在头显里输入正确  IP 无法连接）
